...
void solve()
{
    ...
    for(int i = 0 ; i < n ; i++){
        ...
        if(danger.count(curr)){
            ...
            set<int> new_factors;
            new_danger.insert(x);
            new_factors.insert(1);

            swap(danger, new_danger);
            swap(factors, new_factors);
            ...
            danger.insert(x/curr);

            set<int> new_factors = factors;
            for(auto f : factors){
                int new_factor = f * curr;
                if(new_factor > x) break;
                if(x % new_factor == 0){
                    new_factors.insert(new_factor);
                    danger.insert(x / new_factor);
                }
            }

            new_factors.insert(curr);
            swap(factors, new_factors);
        }
